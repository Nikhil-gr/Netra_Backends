...
export default async function analyzeController(
  req,
  res
) {
  const {
  ...
  try {
    ...
    // Only create a user-linked
    // VisionAnalysis when a real
    // userId was supplied.
    if (
      mode === "describe" &&
      userId
      ...
      if (
        ...
        try {
          ...
          data.visionAnalysisSaved =
            true;

          data.visionAnalysisId =
            analysis._id.toString();
        } catch (error) {
          console.error(
            "Vision analysis save failed:",
            error
          );
